...
function Modal({ onClose, onAdd }: ModalProps) {
  ...
  async function handlePickCandidate(candidate: { url: string; title: string | null }) {
    setLoading(true);
    setError(null);
    try {
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between p-5 border-b border-border">
          <h2 className="font-display text-lg font-semibold">Add a new feed</h2>
...
export function AddFeedDialog({ onAdd }: Props) {
  ...
  return (
    <>
      ...
      {open && <Modal onClose={() => setOpen(false)} onAdd={onAdd} />}
    </>
  );
}
